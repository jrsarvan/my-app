node{
   stage('SCM Checkout'){
     git 'https://github.com/jrsarvan/my-app.git'
   }
   ...
   sh 'docker build -t jrsarvan/myweb:0.0.2 .'
   }
   stage('Docker Image Push'){
   withCredentials([string(credentialsId: 'dockerPass', variable: 'dockerPassword')]) {
   sh "docker login -u jrsarvan -p ${dockerPassword}"
    }
   sh 'docker push jrsarvan/myweb:0.0.2'
   }
